...
m24_treat_ct_act_chart_visual <- function(input_df, fill_hex, sub_label){
  ...
  m24_treat_ct_act_chart <- ggplot(chart_df, aes(x = Total)) +
    geom_histogram(fill = fill_hex, alpha = 0.5, color = "#000000", bins = 20) +
    labs(x = "CT Scan Activity",
         y = "Number of Trials",
         title = "Histogram of in year 24 Month follow-up CT Activity",
         subtitle = sub_label,
...
m24_treat_pos_outcomes_chart_visual <- function(input_df, fill_hex, sub_label){
  
  chart_df <- input_df %>%
    group_by(Trial, m24_treat_outcome_clean) %>%
    summarise(Total = sum(Total, na.rm = TRUE)) %>%
    ungroup() %>%
    complete(Trial, m24_treat_outcome_clean, fill = list(Total = 0))
  
  m24_treat_pos_outcomes_chart <- ggplot(chart_df, aes(x = Total)) +
    geom_histogram(fill = fill_hex, alpha = 0.5, color = "#000000", bins = 20) +
    facet_wrap(~m24_treat_outcome_clean, scales = "free_x") +
...
m24_treat_diags_chart_visual <- function(input_df, fill_hex, sub_label){
  ...
  m24_treat_diags_chart <- ggplot(chart_df, aes(x = Total)) +
...
m24_treat_malig_chart_visual <- function(input_df, fill_hex, sub_label){
  ...
  m24_treat_malig_chart <- ggplot(chart_df, aes(x = Total)) +
    geom_histogram(fill = fill_hex, alpha = 0.5, color = "#000000", bins = 20) +
...
m24_treat_mods_chart_visual <- function(input_df, fill_hex, sub_label){
