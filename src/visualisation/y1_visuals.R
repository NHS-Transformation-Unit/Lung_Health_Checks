...
y1_m3_treat_ct_act_hist_df <- y1_m3_treat_groups_list[[1]] %>%
  filter(m3_rebook != "Opt-out") %>%
  group_by(Trial) %>%
  summarise(Total = sum(Total))

y1_m3_treat_ct_act_hist <- m3_treat_ct_act_chart_visual(y1_m3_treat_ct_act_hist_df, "#407EC9", "Year 1")
...
y1_m3_modalities_hist_df <- y1_m3_diags_modalities_df %>%
  gather(modality, Total, -c(1)) %>%
  mutate(modality_clean = case_when(modality == "bronch" ~ "Bronchoscopy",
                                    modality == "ct_bio" ~ "CT Guided Biopsy",
                                    modality == "ebus" ~ "EBUS",
...
y1_m3_treat_diags_hist <- m3_treat_diags_chart_visual(y1_m3_modalities_hist_df, "#407EC9", "Year 1")


# Month 3 Treatment Confirmed Malignancy ----------------------------------

y1_m3_treat_malig_hist_df <- y1_m3_treat_groups_list[[5]]

y1_m3_treat_malig_hist <- m3_treat_malig_chart_visual(y1_m3_treat_malig_hist_df, "#407EC9", "Year 1")


# Month 3 Treatment Modalities --------------------------------------------

y1_m3_mods_hist_df <- y1_m3_modalities_df %>%
  mutate(modality_clean = case_when(modality == "Best_Supportive_Care" ~ "Best Supportive Care",
                                    modality == "Neoadjuvant_Immunotherapy" ~ "Neoadjuvant Immunotherapy",
                                    modality == "Surgery_Adjuvant_Chemotherapy" ~ "Surgery with Adjuvant Chemotherapy",
                                    modality == "XRT" ~ "Radiotherapy",
...
y1_m3_treat_mods_hist <- m3_treat_mods_chart_visual(y1_m3_mods_hist_df, "#407EC9", "Year 1")

# Month 12 Treatment CT Activity -------------------------------------------

y1_m12_treat_ct_act_hist_df <- y1_m12_treat_groups_list[[1]] %>%
  filter(m12_rebook != "Opt-out") %>%
  group_by(Trial) %>%
  summarise(Total = sum(Total))

y1_m12_treat_ct_act_hist <- m12_treat_ct_act_chart_visual(y1_m12_treat_ct_act_hist_df, "#407EC9", "Year 1")
...
y1_m12_modalities_hist_df <- y1_m12_diags_modalities_df %>%
  gather(modality, Total, -c(1)) %>%
  mutate(modality_clean = case_when(modality == "bronch" ~ "Bronchoscopy",
                                    modality == "ct_bio" ~ "CT Guided Biopsy",
                                    modality == "ebus" ~ "EBUS",
...
y1_m12_treat_diags_hist <- m12_treat_diags_chart_visual(y1_m12_modalities_hist_df, "#407EC9", "Year 1")


# Month 12 Treatment Confirmed Malignancy ----------------------------------

y1_m12_treat_malig_hist_df <- y1_m12_treat_groups_list[[5]]

y1_m12_treat_malig_hist <- m12_treat_malig_chart_visual(y1_m12_treat_malig_hist_df, "#407EC9", "Year 1")


# Month 12 Treatment Modalities --------------------------------------------

y1_m12_mods_hist_df <- y1_m12_modalities_df %>%
  mutate(modality_clean = case_when(modality == "Best_Supportive_Care" ~ "Best Supportive Care",
                                    modality == "Neoadjuvant_Immunotherapy" ~ "Neoadjuvant Immunotherapy",
                                    modality == "Surgery_Adjuvant_Chemotherapy" ~ "Surgery with Adjuvant Chemotherapy",
                                    modality == "XRT" ~ "Radiotherapy",
...
y1_m12_treat_mods_hist <- m12_treat_mods_chart_visual(y1_m12_mods_hist_df, "#407EC9", "Year 1")

# Month 24 Treatment CT Activity -------------------------------------------

y1_m24_treat_ct_act_hist_df <- y1_m24_treat_groups_list[[1]] %>%
  filter(m24_rebook != "Opt-out") %>%
  group_by(Trial) %>%
  summarise(Total = sum(Total))

y1_m24_treat_ct_act_hist <- m24_treat_ct_act_chart_visual(y1_m24_treat_ct_act_hist_df, "#407EC9", "Year 1")
...
y1_m24_modalities_hist_df <- y1_m24_diags_modalities_df %>%
  gather(modality, Total, -c(1)) %>%
  mutate(modality_clean = case_when(modality == "bronch" ~ "Bronchoscopy",
                                    modality == "ct_bio" ~ "CT Guided Biopsy",
                                    modality == "ebus" ~ "EBUS",
...
y1_m24_treat_diags_hist <- m24_treat_diags_chart_visual(y1_m24_modalities_hist_df, "#407EC9", "Year 1")


# Month 24 Treatment Confirmed Malignancy ----------------------------------

y1_m24_treat_malig_hist_df <- y1_m24_treat_groups_list[[5]]

y1_m24_treat_malig_hist <- m24_treat_malig_chart_visual(y1_m24_treat_malig_hist_df, "#407EC9", "Year 1")


# Month 24 Treatment Modalities --------------------------------------------

y1_m24_mods_hist_df <- y1_m24_modalities_df %>%
  mutate(modality_clean = case_when(modality == "Best_Supportive_Care" ~ "Best Supportive Care",
                                    modality == "Neoadjuvant_Immunotherapy" ~ "Neoadjuvant Immunotherapy",
                                    modality == "Surgery_Adjuvant_Chemotherapy" ~ "Surgery with Adjuvant Chemotherapy",
                                    modality == "XRT" ~ "Radiotherapy",
...
y1_m24_treat_mods_hist <- m24_treat_mods_chart_visual(y1_m24_mods_hist_df, "#407EC9", "Year 1")
