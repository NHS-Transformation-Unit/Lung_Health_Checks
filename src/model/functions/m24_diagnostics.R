m24_diagnostic_modalities <- function(input_df, pet_rate, bronch_rate, ebus_rate, ct_bio_rate, nav_bronch_rate, lft_rate){
  
  ### Simulate diagnostic modalitiy requirements
  m24_diag_modalities_df <- input_df %>%
  ...
  m24_diag_modalities_agg_df <- m24_diag_modalities_df %>%
    group_by(Trial) %>%
